...
/// Trích subtopic (level 1) từ form_data — e.g. "Electronics & Devices", "Video"
fn extract_subtopic(form_data: &serde_json::Value) -> Option<&str> {
    form_data
        .get("selection")
        .and_then(|s| {
            s.get("subtopic")
...
/// Trích sub_category (level 2) từ form_data — e.g. "Smartphones", "Short Clips"
fn extract_sub_category(form_data: &serde_json::Value) -> Option<&str> {
    form_data
        .get("selection")
        .and_then(|s| s.get("sub_category"))
        .and_then(|v| v.as_str())
}
...
pub async fn post_offer(
    ...
    let matched = {
        ...
        let new_subtopic = extract_subtopic(&body.form_data);
        let new_sub_category = extract_sub_category(&body.form_data);
        store.iter().find(|o| {
            o.topic == body.topic
                && o.action == opposite
                && o.session_id != body.session_id
                // Level 1: subtopic phải trùng nếu cả hai có
                && match (new_subtopic, extract_subtopic(&o.form_data)) {
                    (Some(a), Some(b)) => a == b,
                    _ => true,
                }
                // Level 2: sub_category phải trùng nếu cả hai có
                && match (new_sub_category, extract_sub_category(&o.form_data)) {
                    (Some(a), Some(b)) => a == b,
                    _ => true,
                }
        }).cloned()
    ...
    // Lưu offer vào DB (xóa offer cũ của cùng session+topic trước)
    sqlx::query("DELETE FROM offers WHERE session_id = $1 AND topic = $2")
    ...
    sqlx::query(
        "INSERT INTO offers (offer_id, session_id, topic, action, form_data, timestamp)
         VALUES ($1, $2, $3, $4, $5, $6)"
    )
    .bind(&offer_id)
    .bind(&body.session_id)
    ...
    // Announce lên tracker để dashboard subtopic hiện đúng số người
    // Trích subtopic từ form_data.selection.subtopic (cấu trúc DynamicForm)
    {
        let subtopic = body.form_data
            .get("selection").and_then(|s| s.get("subtopic")).and_then(|v| v.as_str())
            .or_else(|| body.form_data.get("selection").and_then(|s| s.get("category")).and_then(|v| v.as_str()))
            .or_else(|| body.form_data.get("subtopic").and_then(|v| v.as_str()))
            .map(|s| s.to_string());

        let mut tracker = state.tracker_store.write().await;
        let entries = tracker.entry(body.topic.clone()).or_default();
        // Xóa entry cũ của session này
        entries.retain(|e| e.node_id != body.session_id);
        entries.push(crate::state::TopicPeerEntry {
            node_id: body.session_id.clone(),
            endpoint_addr: serde_json::Value::Null,
    ...
    // Push WsEvent để dashboard cập nhật realtime
    {
        let stats = crate::routes::tracker::compute_subtopic_stats_pub(&state, &body.topic).await;
...
#[derive(Deserialize)]
pub struct CancelOfferBody {
    pub session_id: String,
    pub topic: String,
}

/// DELETE /api/offers — Hủy offer khi deal xong, xóa khỏi store + tracker
pub async fn cancel_offer(
    State(state): State<Arc<AppState>>,
    Json(body): Json<CancelOfferBody>,
) -> Json<serde_json::Value> {
    // Xóa khỏi DB
    let _ = sqlx::query("DELETE FROM offers WHERE session_id = $1 AND topic = $2")
        .bind(&body.session_id)
        .bind(&body.topic)
        .execute(&state.db)
        .await;

    // Xóa khỏi in-memory store
    {
        let mut store = state.offer_store.write().await;
        store.retain(|o| !(o.session_id == body.session_id && o.topic == body.topic));
    }
    // Xóa khỏi tracker_store
    {
        let mut tracker = state.tracker_store.write().await;
    ...
    Json(json!({ "status": "cancelled", "session_id": body.session_id }))
}
