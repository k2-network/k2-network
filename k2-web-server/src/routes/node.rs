...
pub async fn init_node(State(state): State<Arc<AppState>>) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    // Guard: already initialized
    {
        ...
        if let Some(ref node) = *guard {
            ...
            return Ok(Json(json!({ "node_id": node_id, "status": "existing" })));
        }
    }
    ...
    Ok(Json(json!({ "node_id": node_id, "status": "initialized" })))
}

/// GET /api/node-id — Get full node ID (iroh, public)
...
/// GET /api/user/node-id — Node ID cố định của user đang đăng nhập (lấy từ DB, không đổi)
pub async fn get_user_node_id(
