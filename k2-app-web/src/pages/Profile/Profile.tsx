...
import { getMyNodeId, getMySessionId } from "../../api";
    ...
    return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
            ...
            {stats.map((s, i) => {
                ...
                return (
                    <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fill="#d3d3d3" fontSize="10">
                        {s.label}
                    </text>
                );
            })}
...
export function ProfilePage() {
    ...
    useEffect(() => {
        if (mode === "auth" && user) {
            // Logged-in: dùng node ID từ account
            setNodeId(user.nodeId);
        } else {
            // Guest: dùng session_id riêng của tab này
            setNodeId(getMySessionId());
        }
    }, [mode, user]);
