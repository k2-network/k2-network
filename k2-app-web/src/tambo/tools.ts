 ...
 */
import { z } from "zod";
import { classifyK2Endpoint, classifyIntent as apiClassifyIntent, startListening, listenOffers, broadcastOffer as apiBroadcastOffer } from "../api";
...
async function extractMarketplaceIntentK2(userPrompt: string): Promise<any> {
    ...
    const rawResult = await classifyK2Endpoint(userPrompt);
    ...
    const actionData = (data.action || {}) as Record<string, string>;
    ...
    const result = {
        ...
        action: ((data.action_type as string) || "sell").toLowerCase(),
...
async function extractMarketplaceIntentTauri(userPrompt: string): Promise<any> {
    console.log("🔧 [API] Calling backend classify_intent...");
    ...
    const result = await apiClassifyIntent({ user_prompt: userPrompt, api_key: apiKey, base_url: baseUrl, model });
...
Sử dụng sau khi đã phân tích intent và người dùng xác nhận muốn tạo yêu cầu.`,
    ...
    tool: async (input: {
        ...
        try {
            ...
            await startListening(input.topic);

            // 2. Broadcast offer to P2P network
            const result = await apiBroadcastOffer(input.topic, formData);
