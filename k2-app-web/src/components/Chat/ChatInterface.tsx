...
import { IoClose, IoSettingsOutline, IoTrashOutline } from 'react-icons/io5';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ isOpen, onToggle, width, onWidthChange, sessionId, hideFab }) => {
  ...
  const { messages, sendMessage, isProcessing, isLoadingHistory, resetChat, deleteMessage } = useGroqChat(sessionId);
  ...
  // Khi AI tạo form mới với intent khác → xóa button/result cũ
  const lastIntentRef = useRef<{ action: string; topic: string | null; subtopic?: string; sub_category?: string } | null>(null);
  useEffect(() => {
    const handler = (event: CustomEvent) => {
      const newIntent = event.detail;
      const prev = lastIntentRef.current;
      const isDifferent = !prev ||
        prev.action !== newIntent.action ||
        prev.topic !== newIntent.topic ||
        prev.subtopic !== newIntent.subtopic ||
        prev.sub_category !== newIntent.sub_category;
      if (isDifferent) {
        setInjectedMessages([]);
      }
      lastIntentRef.current = newIntent;
    };
    window.addEventListener('k2:intentChanged' as any, handler);
    return () => window.removeEventListener('k2:intentChanged' as any, handler);
  }, []);
  ...
  const aiAgentIcon = isLightTheme ? aiAgentIconLight : aiAgentIconDark;

  // Button "Bắt đầu đàm phán" luôn pinned ở trên — lấy cái mới nhất
  const startButtons = injectedMessages.filter(m => m.type === 'startButton');
  const pinnedStartButton = startButtons.length > 0 ? startButtons[startButtons.length - 1] : null;

  // Chỉ chat messages + negotiation results trong scroll list
  const allItems = [
    ...messages.map(msg => ({
      id: msg.id,
    ...
    ...injectedMessages
      .filter(inj => inj.type !== 'startButton')
      .map(inj => ({
        id: inj.id,
        type: inj.type,
        data: inj.content,
  ...
  return (
    <>
      ...
      {isOpen && (
        <div ref={panelRef} className={`chat-panel ${isLightTheme ? 'light' : 'dark'}`} style={{ width: `${width}px` }}>
          ...
          <div className="chat-header">
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              {messages.length > 0 && (
                <button className="chat-settings-btn" onClick={resetChat} title="Xóa toàn bộ tin nhắn">
          ...
          {showSettings ? (
            <div className="api-key-overlay">
              <div className="settings-section">
                ...
                <p className="settings-hint">Mặc định dùng key hệ thống. Bạn có thể nhập key riêng để thay thế.</p>
                <input
                  type="password"
                  className="api-key-input"
                  placeholder="Nhập key riêng (tùy chọn)..."
                ...
                <button
                  ...
                  disabled={!customKeyInput.trim() || keySaveStatus === 'saving'}
                  onClick={async () => {
                    if (!customKeyInput.trim()) return;
                    setKeySaveStatus('saving');
                    try {
                      const { saveUserApiKey } = await import('../../api/ai');
                      await saveUserApiKey(sessionId || 'default', customKeyInput.trim());
                      setCustomKeyInput('');
                ...
                >
                  {keySaveStatus === 'saving' ? 'Đang lưu...' : keySaveStatus === 'saved' ? '✓ Đã lưu' : keySaveStatus === 'error' ? '✗ Lỗi' : 'Lưu key riêng'}
                </button>
              </div>
            </div>
          ) : (
            <>
              {pinnedStartButton && (
                <div className="chat-pinned-action">
                  <StartTransactionButton
                    actionText={pinnedStartButton.content.actionText}
                    title={pinnedStartButton.content.title}
                  />
                </div>
              )}
              <div className="chat-messages">
                {isLoadingHistory ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '32px 0', color: 'var(--color-muted)', fontSize: 13 }}>
                    <div style={{ width: 16, height: 16, border: '2px solid var(--color-primary)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.8s linear infinite', flexShrink: 0 }} />
                    Đang tải lịch sử...
                  </div>
                ) : allItems.length === 0 ? (
                  ...
                  allItems.map((item) => {
                    if (item.type === 'chatMessage') {
                      const msg = item.data;
                      return (
                        <div key={item.id} className={`message ${msg.role} message-deletable`}>
                          {msg.role === 'assistant' && (
